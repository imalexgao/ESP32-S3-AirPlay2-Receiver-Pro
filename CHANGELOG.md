...
> 通用版专属升级（KEF 有源音箱专版为固定配置，不包含格式选择）。
...
- **低延时模式（可选）**：调小播放缓冲，目标端到端延时更低（代价：WiFi 波动时更易出现卡顿/爆音）；默认关闭，保持稳定优先
...
- AirPlay 端到端延时受协议与网络限制：1 ms 级只可能指"多设备同步误差"（AirPlay 2 NTP 时钟同步），端到端（点击到出声）目标为低延时模式下 ~120–150 ms 量级，无法做到 1 ms（见 README「延时说明」）
